...
class GameboardUi extends Gameboard {
  constructor(name) {
    ...
    super(4); this.name = name;this.count = 0;
  ...
  placeShip(shipPara, xCoordinate, yCoordinate) {
    let ship = GameboardUi.#createShips(shipPara.length, shipPara.place, this);
    ...
    if (
      ...
      for (i = 0; i < ship.length; i++) {
        ...
        this.grid[xCoordinate + cvrHrz][yCoordinate + cvrVer].dataset.shipId =
          ship.id;
        ...
        //add ship class
        this.grid[xCoordinate + cvrHrz][yCoordinate + cvrVer].classList.add(
          "ship",
        );
      }
    }
  ...
  receiveAttack(xCoordinate, yCoordinate) {
    if (this.placeOccupied.has([xCoordinate, yCoordinate].toLocaleString())) {
      ...
      let attackedShip = this.revealAttackedShip(attackedCoordinateShipIdValue);
  ...
  shipSunked(ship) {
    ...
    alert(`destroyed ${  ++this.count } ${this.name} ship completely ${this.ships.length-this.count}remaining`) 
  } // ui logic ends;
  ...
  allShipSunked() { 
    ...
    this.ships.forEach((s) => {
      ...
      if (!s.sunked) {allSunked = false;console.log('df',s.sunked)} //even if s.sunked = false allsunked is false
